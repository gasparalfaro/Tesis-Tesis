...
\begin{figure}
    ...
    \caption{Snapshots at $4$ different times of both configurations (original, and copy with altered agent at $t = 0$) of the public goods game. Cooperators are plotted in blue, defectors in red. Both configurations are plotted one on top of the other with a transparent effect. Since both configurations are the same, only those cells that differ change with time. Thanks to this we can see that the mismatches propagate at average constant velocity and expand radially.}
